...
const renderTheme = themes => {
   ...
   for (let i = 0; i < themes.length; i++) {
       ...
       const button = document.createElement('button');
        ...
        p.innerHTML = `
        <span class="selection__result__ratio">${result}/${themes[i].list.length}</span>
        <span class="selection__result__text">Отличный результат</span>
        `;
        li.append(p);
        }
...
const showResult = (result, quiz) => {
    ...
    block.className = 'main__box main__box_result';
